-- 신고/어드민 시스템 전용 DB 스키마 생성 및 초기 설정값 등록 마이그레이션 스크립트
...
CREATE TABLE reports (
    id UUID PRIMARY KEY,
    report_code VARCHAR(30) DEFAULT ('RPT-' || lpad(CAST(nextval('report_code_seq') AS VARCHAR), 4, '0')) NOT NULL UNIQUE,
    ...
    details TEXT,
    status VARCHAR(20) DEFAULT 'RECEIVED' NOT NULL, -- RECEIVED, IN_REVIEW, RESOLVED, REJECTED
...
CREATE TABLE user_penalties (
    ...
    end_at TIMESTAMP WITH TIME ZONE, -- NULL이면 영구 정지
