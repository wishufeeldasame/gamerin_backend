...
- 경량 보안 스캔으로 실행 파일 헤더와 EICAR 테스트 시그니처 차단
...
- 동영상 최적화 결과처럼 임시 파일 경로를 저장하기 위한 `PreparedMediaPath` record와 저장 오버로드를 추가했다.
...
- 동영상 업로드는 보안 검증 후 FFmpeg 기반 빠른 MP4 최적화 결과를 저장
...
- 동영상 저장 테스트는 최적화된 `PreparedMediaPath` 저장 흐름을 검증하도록 수정
...
현재는 비용을 줄이기 위해 실행 파일 헤더와 EICAR 테스트 시그니처를 막는 경량 스캔을 사용한다. OpenAI moderation은 악성코드 스캐너가 아니며, 실제 운영 규모가 커지면 ClamAV 같은 별도 스캐너를 붙이는 편이 좋다.
